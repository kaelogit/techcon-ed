...
export function ImpactStoriesSection() {
  ...
  // Slowed down the interval to 15 seconds to give people time to read the deep stories
  useEffect(() => {
    if (isPaused) return;
  ...
  return (
    <View className="py-24 md:py-32 px-6 bg-edwin-navy border-t border-edwin-black relative overflow-hidden">
      
      {/* Background ambient glow to make it feel premium */}
      <View className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-5xl h-full bg-blue-900/10 blur-[120px] rounded-full pointer-events-none" />
      ...
      <View className="max-w-5xl mx-auto relative z-10">
        ...
        <View className="text-center mb-16">
          ...
          <Text className="block text-4xl md:text-5xl font-light text-white leading-tight animate-on-load delay-100">
          Rebuilding futures, one family at a time.
          </Text>
        </View>

      {/* The Interactive Story Card */}
        <div
          className="bg-white rounded-3xl p-8 md:p-14 shadow-2xl border border-stone-100 animate-on-load delay-200"
          onMouseEnter={() => setIsPaused(true)}
          onMouseLeave={() => setIsPaused(false)}
          ref={containerRef}
        >
          {/* Top Info Bar: Category, Amount, Name */}
          <View className="flex flex-col md:flex-row md:items-center justify-between border-b border-stone-200 pb-8 mb-8 gap-6">
            <View>
              <Text className="block text-xl md:text-2xl font-bold text-edwin-navy mb-2">
                {currentStory.category}
              </Text>
              <View className="flex items-center gap-2">
                <Text className="text-base font-bold text-edwin-black">{currentStory.name}</Text>
                <Text className="text-stone-400">—</Text>
                <Text className="text-base text-stone-500">{currentStory.location}</Text>
              </View>
            </View>
            <View className="bg-emerald-50 border border-emerald-100 px-6 py-3 rounded-xl self-start md:self-auto">
              <Text className="block text-xs font-bold text-emerald-600 uppercase tracking-widest mb-1">
                Funding Provided
              </Text>
              <Text className="block text-2xl md:text-3xl font-extrabold text-emerald-700">
                {currentStory.amount}
              </Text>
            </View>
          </View>

          {/* The Full Story Content with Fade Transition */}
          <View className={`transition-opacity duration-300 ${isFading ? 'opacity-0' : 'opacity-100'}`}>
            <View className="space-y-6 mb-12">
              {currentStory.paragraphs.map((paragraph, idx) => {
                // Check if this is the final quote paragraph to emphasize it
                if (idx === currentStory.paragraphs.length - 1 && paragraph.startsWith("“")) {
                  return (
                    <Text key={idx} className="block text-xl md:text-2xl font-light italic text-edwin-black leading-relaxed mt-10 border-l-4 border-amber-400 pl-6">
                      {paragraph}
                    </Text>
                  );
                }
                return (
                  <Text key={idx} className="block text-base md:text-lg text-stone-600 leading-relaxed">
                    {paragraph}
                  </Text>
                );
              })}
          ...
          <View className="flex items-center justify-between border-t border-stone-100 pt-8">
            <Text className="text-sm font-bold tracking-widest text-stone-400 uppercase">
              Story {currentIndex + 1} of {stories.length}
            </Text>
            <View className="flex gap-4">
              <button
                onClick={handlePrev}
                className="w-12 h-12 rounded-full border border-stone-200 flex items-center justify-center text-edwin-black hover:bg-stone-50 hover:border-stone-300 transition-all"
              ...
              <button
                onClick={handleNext}
                className="w-12 h-12 rounded-full bg-edwin-navy text-white flex items-center justify-center hover:bg-edwin-black transition-all shadow-sm"
